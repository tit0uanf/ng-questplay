...
use src::contracts::brainfuck_vm::{BrainfuckVM, IBrainfuckVMDispatcher};

fn assert_array<T, impl TPartialEq: PartialEq<T>, impl TDrop: Drop<T>>(
    mut actual: Array<T>, mut expected: Array<T>
) {
    ...
    while (actual.len() > 0) {
        ...
        assert(actual_item == expected_item, 'Unexpected item');
    }
}
...
fn deploy_brainfuck_vm() -> IBrainfuckVMDispatcher {
    ...
    let (vm_address, _) = deploy_syscall(vm_hash, 0, ArrayTrait::new().span(), false).unwrap();

    IBrainfuckVMDispatcher { contract_address: vm_address }
}
